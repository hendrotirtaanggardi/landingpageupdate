    ...
    <div class="card-body p-lg-17">
            ...
            <div class="position-relative mb-17">
                ...
                <div class="position-absolute text-white mb-8 ms-10 bottom-0">
                    <h3 class="text-white fs-2qx fw-bolder mb-3 m">
                        {{ __('📦 Manage Framework and Library') }}
                    </h3>
                    <div class="fs-5 fw-bold">
                        {{ __('You can manage Framework and Libraries that talent can pick here!') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex flex-column flex-lg-row mb-17">
            <div class="flex-lg-row-fluid me-0">
                ...
                @if ($editClicked)
                <livewire:framework-library-update />
                @else
                <livewire:framework-library-create />
                @endif
                    ...
                    <div class="col-md-12">
                        <div class="table-responsive mb-15 border">
                            <table class="table table-row-bordered gy-7 gs-7">
                                ...
                                <tbody>
                                    @forelse ($framework_libraries as $framework_library)
                                    <tr>
                                        <td>{{ ($framework_libraries->currentPage() - 1) *
                                            $framework_libraries->perPage() + $loop->iteration }}
                                        </td>
                                        <td>{{ $framework_library->name }}</td>
                                        <td>{{ $framework_library->users->count() }}</td>
                                        <td class="text-end">
                                            <button class="btn btn-light btn-sm me-1 mb-2" type="button"
                                                wire:click="getFrameworkLibrary({{ $framework_library->id }})">✏️</button>
                                            <button class="btn btn-light btn-sm mb-2" type="button"
                                                wire:click="deleteFrameworkLibrary({{ $framework_library->id }})">❌</button>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="4">
                                            <div class="text-center">
                                                <h3 class="text-gray-800">
                                                    {{ __('No Framework or Libray found!') }}
                                                </h3>
                                            </div>
                                        </td>
                                    </tr>
