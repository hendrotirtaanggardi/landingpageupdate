    ...
    <div class="card-body p-lg-17">
            ...
            <div class="position-relative mb-17">
                ...
                <div class="position-absolute text-white mb-8 ms-10 bottom-0">
                    <h3 class="text-white fs-2qx fw-bolder mb-3 m">
                        {{ __('👨‍💻️ Manage Programming Language') }}
                    </h3>
                    <div class="fs-5 fw-bold">
                        {{ __('You can manage programming languages that talent can pick here!') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex flex-column flex-lg-row mb-17">
            <div class="flex-lg-row-fluid me-0">
                ...
                @if ($editClicked)
                <livewire:programming-language-update />
                @else
                <livewire:programming-language-create />
                @endif
                    ...
                    <div class="col-md-12">
                        <div class="table-responsive mb-15 border">
                            <table class="table table-row-bordered gy-7 gs-7">
                                ...
                                <tbody>
                                    @forelse ($programming_languages as $programming_language)
                                    <tr>
                                        <td>{{ ($programming_languages->currentPage() - 1) *
                                            $programming_languages->perPage() + $loop->iteration }}
                                        </td>
                                        <td>{{ $programming_language->name }}</td>
                                        <td>{{ $programming_language->users->count() }}</td>
                                        <td class="text-end">
                                            <button class="btn btn-light btn-sm me-1 mb-2" type="button"
                                                wire:click="getProgrammingLanguage({{ $programming_language->id }})">✏️</button>
                                            <button class="btn btn-light btn-sm mb-2" type="button"
                                                wire:click="deleteProgrammingLanguage({{ $programming_language->id }})">❌</button>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="4">
                                            <div class="text-center">
                                                <h3 class="text-gray-800">
                                                    {{ __('No programming language found!') }}
                                                </h3>
                                            </div>
                                        </td>
                                    </tr>
